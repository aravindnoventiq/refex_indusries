/**
 * Migration script to download all PDFs from registrar-transfer-agent page
 * and save them to the server's uploads/pdfs/registrar-transfer-agent subfolder
 * Also moves any PDFs from root pdfs folder to the subfolder
 * 
 * This script:
 * 1. Fetches the registrar-transfer-agent page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/registrar-transfer-agent/
 * 3. Moves any PDFs from /uploads/pdfs/ to /uploads/pdfs/registrar-transfer-agent/
 * 4. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-registrar-transfer-agent-pdfs.js
 */
...
async function migrateRegistrarTransferAgent() {
  console.log('='.repeat(70));
  console.log('Starting Registrar Transfer Agent PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the registrar-transfer-agent page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_registrar_transfer_agent.js');
      return;
    }
