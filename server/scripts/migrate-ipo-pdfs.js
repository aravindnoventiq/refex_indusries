/**
 * Migration script to download all PDFs from ipo page
 * and save them to the server's uploads/pdfs/ipo subfolder
 * 
 * This script:
 * 1. Fetches the ipo page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/ipo/
 * 3. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-ipo-pdfs.js
 */
...
// Ensure uploads/pdfs/ipo directory exists
...
async function migrateIPO() {
  console.log('='.repeat(70));
  console.log('Starting IPO PDF Migration');
  console.log('='.repeat(70));
  ...
  try {
    ...
    if (!pageContent) {
      console.log('❌ No ipo page found in database');
      console.log('Please run the seed script first: node scripts/seed_investors_ipo.js');
      return;
    }
