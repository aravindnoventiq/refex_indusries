/**
 * Migration script to download all PDFs from statement-of-deviation page
 * and save them to the server's uploads/pdfs/statement-of-deviation subfolder
 * Also moves any PDFs from root pdfs folder to the subfolder
 * 
 * This script:
 * 1. Fetches the statement-of-deviation page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/statement-of-deviation/
 * 3. Moves any PDFs from /uploads/pdfs/ to /uploads/pdfs/statement-of-deviation/
 * 4. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-statement-of-deviation-pdfs.js
 */
...
async function migrateStatementOfDeviation() {
  console.log('='.repeat(70));
  console.log('Starting Statement of Deviation PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the statement-of-deviation page
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'statement-of-deviation' }
    });

    if (!pageContent) {
      console.log('❌ No statement-of-deviation page found in database');
      console.log('Please run the seed script first: node scripts/seed_investors_statement_of_deviation.js');
      return;
    }
