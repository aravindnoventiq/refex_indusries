...
const pageData = require("../seeds/investors_book_closure.json");

async function seedInvestorsBookClosure() {
  try {
    console.log("Starting to seed Investors Book Closure page...");
    ...
    if (!created) {
      ...
      console.log(`✓ Book Closure page updated: ${page.title}`);
    } else {
      console.log(`✓ Book Closure page created: ${page.title}`);
    }
    ...
    console.log(`✓ Book Closure page seeded successfully!`);
    await sequelize.close();
    process.exit(0);
  } catch (error) {
    console.error("Error seeding Investors Book Closure page:", error);
    await sequelize.close();
    process.exit(1);
  }
}
...
if (require.main === module) {
  seedInvestorsBookClosure();
}

module.exports = seedInvestorsBookClosure;
