 ...
 * and save them locally for Financial Information page
 */
...
const SUBFOLDER = 'financial-information';
...
async function migratePdfs() {
  try {
    console.log('Starting Financial Information PDF migration...\n');
    ...
    if (!pageContent) {
      console.log('Page content not found for financial-information');
    ...
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
      ...
      for (let docIndex = 0; docIndex < documents.length; docIndex++) {
        ...
        // Check if it's an external URL (skip example.com placeholder URLs)
        if (doc.pdfUrl && (doc.pdfUrl.startsWith('http://') || doc.pdfUrl.startsWith('https://')) && !doc.pdfUrl.includes('example.com')) {
          ...
          console.log(`Skipping (already local, invalid, or placeholder): ${doc.pdfUrl}`);
        }
      }
    }
