/**
 * Migration script to download all PDFs from stock-exchange-quarterly-compliance page
 * and save them to the server's uploads/pdfs/stock-exchange-quarterly-compliance subfolder
 * Also moves any PDFs from root pdfs folder to the subfolder
 * 
 * This script:
 * 1. Fetches the stock-exchange-quarterly-compliance page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/stock-exchange-quarterly-compliance/
 * 3. Moves any PDFs from /uploads/pdfs/ to /uploads/pdfs/stock-exchange-quarterly-compliance/
 * 4. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-stock-exchange-quarterly-compliance-pdfs.js
 */
...
const SUBFOLDER = 'stock-exchange-quarterly-compliance';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);
const ROOT_PDFS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs');

// Ensure uploads/pdfs/stock-exchange-quarterly-compliance directory exists
...
async function migrateStockExchangeQuarterlyCompliance() {
  console.log('='.repeat(70));
  console.log('Starting Stock Exchange Quarterly Compliance PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the stock-exchange-quarterly-compliance page
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'stock-exchange-quarterly-compliance' }
    });

    if (!pageContent) {
      console.log('❌ No stock-exchange-quarterly-compliance page found in database');
      console.log('Please run the seed script first: node scripts/seed_investors_stock_exchange_quarterly_compliance.js');
      return;
    }
    ...
    console.log(`  Filter Items: ${(pageContent.filterItems || []).join(', ')}`);
    console.log('');
...
// Run the migration
migrateStockExchangeQuarterlyCompliance();
