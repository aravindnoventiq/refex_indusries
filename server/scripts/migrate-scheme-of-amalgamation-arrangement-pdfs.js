/**
 * Migration script to download all PDFs from scheme-of-amalgamation-arrangement page
 * and save them to the server's uploads/pdfs/scheme-of-amalgamation-arrangement subfolder
 * Also moves any PDFs from root pdfs folder to the subfolder
 * 
 * This script:
 * 1. Fetches the scheme-of-amalgamation-arrangement page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/scheme-of-amalgamation-arrangement/
 * 3. Moves any PDFs from /uploads/pdfs/ to /uploads/pdfs/scheme-of-amalgamation-arrangement/
 * 4. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-scheme-of-amalgamation-arrangement-pdfs.js
 */
...
const SUBFOLDER = 'scheme-of-amalgamation-arrangement';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);
const ROOT_PDFS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs');

// Ensure uploads/pdfs/scheme-of-amalgamation-arrangement directory exists
...
async function migrateSchemeOfAmalgamationArrangement() {
  console.log('='.repeat(70));
  console.log('Starting Scheme of Amalgamation Arrangement PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the scheme-of-amalgamation-arrangement page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_scheme_of_amalgamation_arrangement.js');
      return;
    }
...
// Run the migration
migrateSchemeOfAmalgamationArrangement();
