 ...
 * and save them locally for Disclosure of Material Events page
 */
...
async function migratePdfs() {
  try {
    console.log('Starting Disclosure PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'disclosure-of-material-events-or-information' }
    });

    if (!pageContent) {
      console.log('Page content not found for disclosure-of-material-events-or-information');
