 ...
 * and save them locally for CSR Activities page
 */
...
const SUBFOLDER = 'csr-activities';
...
async function migratePdfs() {
  try {
    console.log('Starting CSR Activities PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'csr-activities' }
    });

    if (!pageContent) {
      console.log('Page content not found for csr-activities');
