/**
 * Export current secretarial-compliance-report page data from database to seed JSON
 * This updates the seed file with current database state (including local PDF paths)
 * 
 * Run with: node scripts/export-secretarial-compliance-report-to-seed.js
 */
...
async function exportToSeed() {
  ...
  console.log('Exporting Secretarial Compliance Report to Seed JSON');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const seedPath = path.join(__dirname, '..', 'seeds', 'investors_secretarial_compliance_report.json');
