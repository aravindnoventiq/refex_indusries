 ...
 * and save them locally for Credit Ratings page
 */
...
const SUBFOLDER = 'credit-ratings';
...
async function migratePdfs() {
  try {
    console.log('Starting Credit Ratings PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'credit-ratings' }
    });

    if (!pageContent) {
      console.log('Page content not found for credit-ratings');
