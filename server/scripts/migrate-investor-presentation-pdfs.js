/**
 * Migration script to download all PDFs from investor-presentation page
 * and save them to the server's uploads/pdfs/investor-presentation subfolder
 * 
 * This script:
 * 1. Fetches the investor-presentation page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/investor-presentation/
 * 3. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-investor-presentation-pdfs.js
 */
...
const SUBFOLDER = 'investor-presentation';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);

// Ensure uploads/pdfs/investor-presentation directory exists
...
async function migrateInvestorPresentation() {
  console.log('='.repeat(70));
  console.log('Starting Investor Presentation PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the investor-presentation page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_investor_presentation.js');
      return;
    }
