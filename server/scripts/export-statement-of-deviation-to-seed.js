/**
 * Export current statement-of-deviation page data from database to seed JSON
 * This updates the seed file with current database state (including local PDF paths)
 * 
 * Run with: node scripts/export-statement-of-deviation-to-seed.js
 */
...
async function exportToSeed() {
  ...
  console.log('Exporting Statement of Deviation to Seed JSON');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const seedPath = path.join(__dirname, '..', 'seeds', 'investors_statement_of_deviation.json');
