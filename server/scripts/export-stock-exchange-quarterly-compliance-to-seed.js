/**
 * Export current stock-exchange-quarterly-compliance page data from database to seed JSON
 * This updates the seed file with current database state (including local PDF paths)
 * 
 * Run with: node scripts/export-stock-exchange-quarterly-compliance-to-seed.js
 */
...
async function exportToSeed() {
  ...
  console.log('Exporting Stock Exchange Quarterly Compliance to Seed JSON');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const seedPath = path.join(__dirname, '..', 'seeds', 'investors_stock_exchange_quarterly_compliance.json');
