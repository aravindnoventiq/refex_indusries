 ...
 * and save them locally for Familiarization Programme page
 */
...
const SUBFOLDER = 'familiarization-programme';
...
async function migratePdfs() {
  try {
    console.log('Starting Familiarization Programme PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'familiarization-programme-for-independent-directors' }
    });

    if (!pageContent) {
      console.log('Page content not found for familiarization-programme-for-independent-directors');
