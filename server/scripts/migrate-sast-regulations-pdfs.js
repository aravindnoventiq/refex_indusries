 ...
 * and save them locally for Disclosures under SAST Regulations 2011 page
 */
...
const SUBFOLDER = 'sast-regulations';
...
async function migratePdfs() {
  try {
    console.log('Starting SAST Regulations PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'disclosures-under-sast-regulations-2011' }
    });

    if (!pageContent) {
      console.log('Page content not found for disclosures-under-sast-regulations-2011');
