 ...
 * and save them locally for Financial Statement of Subsidiary page
 */
...
const SUBFOLDER = 'financial-statement-subsidiary';
...
async function migratePdfs() {
  try {
    console.log('Starting Financial Statement of Subsidiary PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'financial-statement-of-subsidiary' }
    });

    if (!pageContent) {
      console.log('Page content not found for financial-statement-of-subsidiary');
