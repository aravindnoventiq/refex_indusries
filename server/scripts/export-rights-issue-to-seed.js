/**
 * Export current rights-issue page data from database to seed JSON
 * This updates the seed file with current database state (including local PDF paths)
 * 
 * Run with: node scripts/export-rights-issue-to-seed.js
 */
...
async function exportToSeed() {
  ...
  console.log('Exporting Rights Issue to Seed JSON');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const seedPath = path.join(__dirname, '..', 'seeds', 'investors_rights_issue.json');
