/**
 * Export current ipo page data from database to seed JSON
 * This updates the seed file with current database state (including local PDF paths)
 * 
 * Run with: node scripts/export-ipo-to-seed.js
 */
...
async function exportToSeed() {
  ...
  console.log('Exporting IPO to Seed JSON');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const seedPath = path.join(__dirname, '..', 'seeds', 'investors_ipo.json');
