/**
 * Migration script to download all PDFs from newspaper-publication page
 * and save them to the server's uploads/pdfs/newspaper-publication subfolder
 * 
 * This script:
 * 1. Fetches the newspaper-publication page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/newspaper-publication/
 * 3. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-newspaper-publication-pdfs.js
 */
...
const SUBFOLDER = 'newspaper-publication';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);

// Ensure uploads/pdfs/newspaper-publication directory exists
...
async function migrateNewspaperPublication() {
  console.log('='.repeat(70));
  console.log('Starting Newspaper Publication PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the newspaper-publication page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_newspaper_publication.js');
      return;
    }
