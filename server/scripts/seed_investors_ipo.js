...
const pageData = require("../seeds/investors_ipo.json");

async function seedInvestorsIPO() {
  try {
    console.log("Starting to seed Investors IPO page...");
    ...
    if (!created) {
      ...
      console.log(`✓ IPO page updated: ${page.title}`);
    } else {
      console.log(`✓ IPO page created: ${page.title}`);
    }
    ...
    console.log(`✓ IPO page seeded successfully!`);
    await sequelize.close();
    process.exit(0);
  } catch (error) {
    console.error("Error seeding Investors IPO page:", error);
    await sequelize.close();
    process.exit(1);
  }
}
...
module.exports = seedInvestorsIPO;
