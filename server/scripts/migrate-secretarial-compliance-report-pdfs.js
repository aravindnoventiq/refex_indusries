/**
 * Migration script to download all PDFs from secretarial-compliance-report page
 * and save them to the server's uploads/pdfs/secretarial-compliance-report subfolder
 * Also moves any PDFs from root pdfs folder to the subfolder
 * 
 * This script:
 * 1. Fetches the secretarial-compliance-report page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/secretarial-compliance-report/
 * 3. Moves any PDFs from /uploads/pdfs/ to /uploads/pdfs/secretarial-compliance-report/
 * 4. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-secretarial-compliance-report-pdfs.js
 */
...
async function migrateSecretarialComplianceReport() {
  console.log('='.repeat(70));
  console.log('Starting Secretarial Compliance Report PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the secretarial-compliance-report page
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'secretarial-compliance-report' }
    });

    if (!pageContent) {
      console.log('❌ No secretarial-compliance-report page found in database');
      console.log('Please run the seed script first: node scripts/seed_investors_secretarial_compliance_report.js');
      return;
    }
      ...
      for (let docIndex = 0; docIndex < documents.length; docIndex++) {
        ...
        if (doc.pdfUrl) {
          if (isExternalUrl(doc.pdfUrl)) {
            ...
            if (doc.publishedDate) console.log(`  Published: ${doc.publishedDate}`);
...
// Run the migration
migrateSecretarialComplianceReport();
