/**
 * Migration script to download all PDFs from monitoring-agency-report page
 * and save them to the server's uploads/pdfs/monitoring-agency-report subfolder
 * 
 * This script:
 * 1. Fetches the monitoring-agency-report page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/monitoring-agency-report/
 * 3. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-monitoring-agency-report-pdfs.js
 */
...
const SUBFOLDER = 'monitoring-agency-report';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);

// Ensure uploads/pdfs/monitoring-agency-report directory exists
...
async function migrateMonitoringAgencyReport() {
  console.log('='.repeat(70));
  console.log('Starting Monitoring Agency Report PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the monitoring-agency-report page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_monitoring_agency_report.js');
      return;
    }
