 ...
 * and save them locally for Employee Stock Option Scheme page
 */
...
async function migratePdfs() {
  try {
    console.log('Starting Employee Stock Option Scheme PDF migration...\n');

    // Get the page content
    const pageContent = await db.InvestorsPageContent.findOne({
      where: { slug: 'employee-stock-option-scheme' }
    });

    if (!pageContent) {
      console.log('Page content not found for employee-stock-option-scheme');
