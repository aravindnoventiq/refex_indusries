/**
 * Migration script to download all PDFs from policies page
 * and save them to the server's uploads/pdfs/policies subfolder
 * 
 * This script:
 * 1. Fetches the policies page from database
 * 2. Downloads all external PDFs to /uploads/pdfs/policies/
 * 3. Updates the database with local file paths
 * 
 * Run with: node scripts/migrate-policies-pdfs.js
 */
...
const SUBFOLDER = 'policies';
const UPLOADS_DIR = path.join(__dirname, '..', 'uploads', 'pdfs', SUBFOLDER);

// Ensure uploads/pdfs/policies directory exists
...
async function migratePolicies() {
  ...
  console.log('Starting Policies PDF Migration');
  console.log('='.repeat(70));
  console.log(`Target folder: /uploads/pdfs/${SUBFOLDER}/`);
  console.log('');

  try {
    // Find the policies page
    ...
    if (!pageContent) {
      ...
      console.log('Please run the seed script first: node scripts/seed_investors_policies.js');
      return;
    }
