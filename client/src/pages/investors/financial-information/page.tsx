...
interface PageContent {
  ...
  filterItems?: string[]; // Array of filter items (years) from CMS
  sections: Section[];
...
export default function FinancialInformationPage() {
    ...
    try {
      ...
      if (isInvestorCmsActive(data)) {
        ...
        if (data.hasYearFilter) {
          // Use CMS-defined filter items if available, otherwise extract from documents
      ...
      console.error('Failed to load financial information page:', err);
  ...
  const getAllYears = (): string[] => {
    // Use CMS-defined filter items if available (handle both camelCase and snake_case)
