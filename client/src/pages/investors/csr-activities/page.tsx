...
export default function CSRActivitiesPage() {
  ...
  useEffect(() => {
    document.title = 'CSR Activities – Refex Industries Ltd.';
    
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute('content', 'CSR Activities for Refex Industries Limited');
    } else {
      const meta = document.createElement('meta');
      meta.name = 'description';
      meta.content = 'CSR Activities for Refex Industries Limited';
  ...
  const loadPageContent = async () => {
    try {
      ...
      console.error('Failed to load CSR Activities page:', err);
          ...
          <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray-600">Loading CSR Activities information...</p>
              </div>
            </div>
          </div>
        </section>
