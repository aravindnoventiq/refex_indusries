...
export default function InvestorIPOCMS() {
  const [pageContent, setPageContent] = useState<IPOPage>({
    slug: 'ipo',
    title: 'IPO',
    documents: [],
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'ipo');
        } catch (err) {
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/ipo`, {
        method: 'POST',
        body: formData,
      });
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`ipo-pdf-upload-${index}`}
            />
            <label
              htmlFor={`ipo-pdf-upload-${index}`}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">IPO Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the IPO section
            </p>
          </div>
