...
interface EmployeeStockOptionSchemePageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorEmployeeStockOptionSchemeCMS() {
  const [pageContent, setPageContent] = useState<EmployeeStockOptionSchemePageContent>({
    slug: 'employee-stock-option-scheme',
    title: 'Employee Stock Option Scheme',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'Employee Stock Option Scheme', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'esop');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Employee Stock Option Scheme', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Employee Stock Option Scheme page');
      }
      setPageContent({
        slug: 'employee-stock-option-scheme',
        title: 'Employee Stock Option Scheme',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Employee Stock Option Scheme page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Employee Stock Option Scheme page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Employee Stock Option Scheme page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Employee Stock Option Scheme', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/esop`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. ESOP Disclosure 2024-25"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`esop-pdf-upload-${index}`}
            />
            <label
              htmlFor={`esop-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Employee Stock Option Scheme page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Employee Stock Option Scheme Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Employee Stock Option Scheme section
            </p>
          </div>
