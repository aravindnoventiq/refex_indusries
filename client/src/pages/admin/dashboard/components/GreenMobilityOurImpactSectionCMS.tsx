...
export default function GreenMobilityOurImpactSectionCMS() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await greenMobilityCmsApi.deleteImpact(id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingImpact.id) {
        await greenMobilityCmsApi.updateImpact(editingImpact.id, editingImpact);
        setSuccess('Impact stat updated successfully');
      } else {
        await greenMobilityCmsApi.createImpact(editingImpact);
  ...
  const handleToggleActive = async (impact: Impact) => {
    try {
      await greenMobilityCmsApi.updateImpact(impact.id!, {
        ...impact,
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., 436.16 lakh / 4.36+ crore, 19+ lakh, 7.6+ lakh"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="e.g., e-Kms covered, Happy Riders, Rides Completed"
                required
              />
            </div>
