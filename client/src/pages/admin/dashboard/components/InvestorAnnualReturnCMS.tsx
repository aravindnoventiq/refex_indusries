...
interface AnnualReturnPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorAnnualReturnCMS() {
  const [pageContent, setPageContent] = useState<AnnualReturnPageContent>({
    slug: 'annual-return',
    title: 'Annual Return',
    hasYearFilter: true,
    filterItems: [],
    sections: [{ title: 'Annual Return', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'annual-return');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Annual Return', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Annual Return page');
      }
      setPageContent({
        slug: 'annual-return',
        title: 'Annual Return',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Annual Return page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Annual Return page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Annual Return page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Annual Return', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/annual-return`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. Annual Return 2024-2025"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`annual-return-pdf-upload-${index}`}
            />
            <label
              htmlFor={`annual-return-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Annual Return page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Annual Return Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Annual Return section
            </p>
          </div>
