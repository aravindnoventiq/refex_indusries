...
interface MonitoringAgencyReportPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorMonitoringAgencyReportCMS() {
  const [pageContent, setPageContent] = useState<MonitoringAgencyReportPageContent>({
    slug: 'monitoring-agency-report',
    title: 'Monitoring Agency Report',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'Monitoring Agency Report', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'monitoring-agency-report');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Monitoring Agency Report', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Monitoring Agency Report page');
      }
      setPageContent({
        slug: 'monitoring-agency-report',
        title: 'Monitoring Agency Report',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Monitoring Agency Report page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Monitoring Agency Report page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Monitoring Agency Report page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Monitoring Agency Report', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/monitoring-agency-report`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. Monitoring Agency Report for the quarter ended 30th June 2024"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`monitoring-agency-pdf-upload-${index}`}
            />
            <label
              htmlFor={`monitoring-agency-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Monitoring Agency Report page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Monitoring Agency Report Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Monitoring Agency Report section
            </p>
          </div>
