...
interface TermsAndConditionsOfAppointmentIDPage {
  id?: number;
  slug: string;
  title: string;
  documents: Document[];
...
export default function InvestorTermsAndConditionsOfAppointmentIDCMS() {
  const [pageContent, setPageContent] = useState<TermsAndConditionsOfAppointmentIDPage>({
    slug: 'terms-and-conditions-of-appointment-of-id',
    title: 'Terms and Conditions of Appointment of ID',
    documents: [],
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'terms-and-conditions-of-appointment-of-id');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      setLoading(true);
      const data = await investorsCmsApi.getPageContentBySlug('terms-and-conditions-of-appointment-of-id');
      if (data) {
  ...
  const handleSave = async () => {
    try {
      ...
      const saveData = {
        ...
        sections: [
          {
            title: 'Terms and Conditions of Appointment',
  ...
  const handleAddDocument = () => {
    setEditingDocument({ title: '', pdfUrl: '', date: '', year: '' });
    setEditingIndex(-1);
    setShowDocumentForm(true);
  };

  const handleEditDocument = (index: number) => {
    setEditingDocument({ ...pageContent.documents[index] });
    setEditingIndex(index);
    setShowDocumentForm(false); // Inline edit
  };

  const handleSaveDocument = (doc: Document, index: number) => {
    const updatedDocs = [...pageContent.documents];
    if (index >= 0) {
      updatedDocs[index] = doc;
    } else {
      updatedDocs.push(doc);
    }
    setPageContent({ ...pageContent, documents: updatedDocs });
  ...
  const handleDeleteDocument = (index: number) => {
    if (window.confirm('Are you sure you want to delete this document?')) {
      const updatedDocs = pageContent.documents.filter((_, i) => i !== index);
      setPageContent({ ...pageContent, documents: updatedDocs });
    }
  };
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/terms-and-conditions-of-appointment-of-id`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. Terms and Condition of Appointment"
          />
        ...
        <div>
          ...
          <div className="relative group">
            <input
              ...
              value={doc.date || ''}
              onChange={(e) => {
                const updatedDoc = { ...doc, date: e.target.value };
                if (index >= 0) {
                  const updatedDocs = [...pageContent.documents];
                  updatedDocs[index] = updatedDoc;
                  setPageContent({ ...pageContent, documents: updatedDocs });
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Terms and Conditions of Appointment CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage appointment document for independent directors
            </p>
          </div>
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-blue-900/5 min-h-[500px]">
              <div className="flex justify-between items-center mb-10">
                <h3 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                  ...
                  Documents List
                  <span className="bg-[#7cd244]/10 text-[#4f8f2a] text-xs px-3 py-1.5 rounded-full font-extrabold">{pageContent.documents.length} Items</span>
                </h3>
                <button
              ...
              {pageContent.documents.length === 0 && !showDocumentForm ? (
