...
interface UnpaidDividendListPageContent {
  id?: number;
  slug: string;
  title: string;
  hasYearFilter: boolean;
  filterItems?: string[];
  sections: { title: string; documents: Document[]; contents?: any[] }[];
  isActive: boolean;
  showPublishDate: boolean;
  showCmsPublishDate: boolean;
}

export default function InvestorUnpaidDividendListCMS() {
  const [pageContent, setPageContent] = useState<UnpaidDividendListPageContent>({
    slug: 'unpaid-dividend-list-and-iepf-shares',
    title: 'Unpaid Dividend List and IEPF Shares',
    hasYearFilter: true,
    filterItems: [],
    sections: [{ title: 'Unpaid Dividend List and IEPF Shares', documents: [] }],
    isActive: true,
  ...
  const [nodalOfficer, setNodalOfficer] = useState<NodalOfficer>({
    name: '',
    designation: '',
    company: '',
    address: '',
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'unpaid-dividend-list-and-iepf-shares');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Unpaid Dividend List and IEPF Shares', documents: [] }],
        ...
        // Extract Nodal Officer from contents if available
        if (data.sections && data.sections[0]?.contents && data.sections[0].contents.length > 0) {
          const content = data.sections[0].contents[0];
          const parsedOfficer = parseContentToNodalOfficer(content);
          setNodalOfficer(parsedOfficer);
        }
      }
    } catch (err: any) {
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Unpaid Dividend List page');
      }
      setPageContent({
        slug: 'unpaid-dividend-list-and-iepf-shares',
        title: 'Unpaid Dividend List and IEPF Shares',
  ...
  const parseContentToNodalOfficer = (content: any): NodalOfficer => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Store Nodal Officer in contents
      const sections = [...pageContent.sections];
      if (!sections[0]) {
        sections[0] = { title: 'Unpaid Dividend List and IEPF Shares', documents: [] };
      }
      sections[0].contents = [
        {
          title: nodalOfficer.name,
          subtitle: nodalOfficer.designation || '',
          content: `${nodalOfficer.company || ''}\n${nodalOfficer.address || ''}\n${nodalOfficer.address2 || ''}\n${nodalOfficer.address3 || ''}\n${nodalOfficer.address4 || ''}\nPhone: ${nodalOfficer.phone || ''}\nEmail: ${nodalOfficer.email || ''}`,
        },
      ];
      ...
      if (pageContent.id) {
        ...
        setSuccess('Unpaid Dividend List page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Unpaid Dividend List page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Unpaid Dividend List page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Unpaid Dividend List and IEPF Shares', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/unpaid-dividend-list-and-iepf-shares`, {
        method: 'POST',
        body: formData,
      });
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`unpaid-dividend-pdf-upload-${index}`}
            />
            <label
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Unpaid Dividend List page...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-blue-900/5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Address Line 2</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Address Line 3</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Address Line 4</label>
                  <input
                    type="text"
