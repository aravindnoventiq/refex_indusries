...
interface InvestorInformationPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorInvestorInformationCMS() {
  const [pageContent, setPageContent] = useState<InvestorInformationPageContent>({
    ...
    if (editingDocument && editingDocument.document) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'investor-information');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Investor Information page');
      }
      setPageContent({
        slug: 'investor-information',
        title: 'Investor Information',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Investor Information page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Investor Information page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Investor Information page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/investor-information`, {
        method: 'POST',
        body: formData,
      });
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`investor-information-pdf-upload-${sectionIndex}-${documentIndex}`}
            />
            <label
              htmlFor={`investor-information-pdf-upload-${sectionIndex}-${documentIndex}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Investor Information page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Investor Information Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage categories and documents for the Investor Information section
            </p>
          </div>
                ...
                <div className="mb-8 p-6 bg-gray-50 rounded-2xl border-2 border-blue-100">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      placeholder="e.g. Formats for Dividend Tax Exemptions"
                    />
                  </div>
