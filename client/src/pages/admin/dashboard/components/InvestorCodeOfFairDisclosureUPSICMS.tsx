...
interface CodeOfFairDisclosureUPSIPage {
  id?: number;
  slug: string;
  title: string;
  documents: Document[];
...
export default function InvestorCodeOfFairDisclosureUPSICMS() {
  const [pageContent, setPageContent] = useState<CodeOfFairDisclosureUPSIPage>({
    slug: 'code-of-fair-disclosure-upsi',
    title: 'Code of Fair Disclosure UPSI',
    documents: [],
    sections: [],
    isActive: true,
  ...
  const handlePdfUrlChange = async (url: string) => {
    ...
    if (isExternalPdfUrl(url) && editingDocument?.document) {
      ...
      try {
        // Download to code-of-fair-disclosure-upsi subfolder
        const response = await fetch(`${API_BASE_URL}/api/download-pdf-from-url/code-of-fair-disclosure-upsi`, {
          method: 'POST',
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/code-of-fair-disclosure-upsi`, {
        method: 'POST',
        body: formData,
      });
  ...
  const loadPageContent = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await investorsCmsApi.getPageContentBySlug('code-of-fair-disclosure-upsi');
      if (data) {
        ...
        if (data.sections && Array.isArray(data.sections) && data.sections.length > 0) {
          ...
          sections = [{ title: 'Code of Fair Disclosure UPSI', documents }];
        }
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load code of fair disclosure UPSI page');
      }
  ...
  const handleSave = async () => {
    try {
      ...
      let existingPage;
      try {
        existingPage = await investorsCmsApi.getPageContentBySlug('code-of-fair-disclosure-upsi');
      } catch (e) {
      ...
      if (existingPage && existingPage.id) {
        ...
        setSuccess('Code of Fair Disclosure UPSI page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Code of Fair Disclosure UPSI page created successfully');
      }
      ...
      setError(err.message || 'Failed to save code of fair disclosure UPSI page');
    }
  };
          ...
          {isManualPdfUrl ? (
            ...
            <div className="animate-in slide-in-from-top-2 duration-300">
              <input
                ...
                id="pdf-upload-standard-inv-code"
                className="hidden"
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">Loading code of fair disclosure UPSI page...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-sm p-6" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                      ...
                      {!isEditing ? (
                        ...
                        <div className="animate-in slide-in-from-top-4 duration-400">
                          <div className="px-6 py-5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white flex justify-between items-center">
                            ...
                            <span className="text-[10px] bg-white/20 px-2 py-1 rounded-full font-bold uppercase tracking-wider">INDEX #{docIndex}</span>
                          </div>
                          <div className="p-8">
      ...
      <div className="flex justify-center mb-10">
        ...
        >
          ...
          <div className="text-center">
            <p className="text-sm font-black text-gray-900 uppercase tracking-tight">Add New Category</p>
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Create a new section for documents</p>
          </div>
        </button>
      </div>
      ...
      {showDocumentForm && editingDocument && editingDocument.documentIndex === -1 && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/20">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-8 py-6 flex justify-between items-center text-white shadow-lg">
              <div className="flex items-center gap-3">
                <div className="bg-white/20 p-2 rounded-xl backdrop-blur-sm">
                  <i className="ri-file-add-line text-2xl"></i>
                </div>
                <div>
                  <h3 className="text-xl font-bold tracking-tight">Add New Document</h3>
                  <p className="text-blue-100 text-xs mt-0.5 opacity-80">Upload documents for the Code of Fair Disclosure UPSI page</p>
                </div>
              </div>
              <button
                onClick={() => { setShowDocumentForm(false); setEditingDocument(null); }}
                className="w-10 h-10 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-full transition-all text-white active:scale-90"
                title="Close"
              >
            ...
            <div className="p-8 bg-gray-50/80 border-t border-gray-100 flex justify-end gap-4 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)]">
              <button
                onClick={() => { setShowDocumentForm(false); setEditingDocument(null); }}
                className="px-8 py-3 bg-white text-gray-600 rounded-xl hover:bg-gray-100 border border-gray-200 transition-all font-bold text-sm shadow-sm active:scale-95"
              >
                Cancel
              </button>
              <button
