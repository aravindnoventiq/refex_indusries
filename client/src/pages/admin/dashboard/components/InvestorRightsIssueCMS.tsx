...
export default function InvestorRightsIssueCMS() {
  const [pageContent, setPageContent] = useState<RightsIssuePage>({
    slug: 'rights-issue',
    title: 'Rights Issue',
    sections: [],
    ...
    if (editingDocument && editingDocument.document) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'rights-issue');
        } catch (err) {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.sections && Array.isArray(data.sections) && data.sections.length > 0) {
          ...
          // Legacy: convert flat documents array to sections
          sections = [{ title: 'Documents', documents: data.documents }];
        }
  ...
  const handleAddDocument = (sectionIndex: number) => {
    setEditingDocument({
      sectionIndex,
      document: { title: '', pdfUrl: '', date: '', year: '' },
      documentIndex: -1,
    });
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/rights-issue`, {
        method: 'POST',
        body: formData,
      });
        ...
        <div className="relative group">
          <input
            ...
            onChange={(e) => {
              ...
              if (documentIndex >= 0) {
                const updatedSections = [...pageContent.sections];
                updatedSections[sectionIndex].documents[documentIndex] = updatedDoc;
                setPageContent({ ...pageContent, sections: updatedSections });
              } else {
                setEditingDocument({ ...editingDocument!, document: updatedDoc });
              }
            ...
            placeholder="e.g. Rights Issue Prospectus"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`rights-issue-pdf-upload-${sectionIndex}-${documentIndex}`}
            />
            <label
              htmlFor={`rights-issue-pdf-upload-${sectionIndex}-${documentIndex}`}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Rights Issue Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Rights Issue section
            </p>
          </div>
                ...
                <div className="mb-8 p-6 bg-gray-50 rounded-2xl border-2 border-blue-100">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      placeholder="e.g. Documents, Issue Documents"
                    />
                  </div>
