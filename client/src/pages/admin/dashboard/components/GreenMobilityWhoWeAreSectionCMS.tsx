...
export default function GreenMobilityWhoWeAreSectionCMS() {
...
Enhance your journey with us and step into a future where sustainability meets innovation.`,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setError('');
      setSuccess('');
      await greenMobilityCmsApi.saveWhoWeAre(section);
  ...
  if (!section) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <p className="text-gray-500">No section data available. Please create one.</p>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            required
          />
          <p className="mt-1 text-xs text-gray-500">Use double line breaks (press Enter twice) to create paragraphs</p>
        </div>
