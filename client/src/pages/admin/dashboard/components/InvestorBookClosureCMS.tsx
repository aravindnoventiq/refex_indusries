...
interface BookClosurePageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorBookClosureCMS() {
  const [pageContent, setPageContent] = useState<BookClosurePageContent>({
    slug: 'book-closure-of-members-register-record-date',
    title: 'Book Closure of Members Register / Record Date',
    hasYearFilter: true,
    filterItems: [],
    sections: [{ title: 'Book Closure of Members Register', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'book-closure');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Book Closure of Members Register', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Book Closure page');
      }
      setPageContent({
        slug: 'book-closure-of-members-register-record-date',
        title: 'Book Closure of Members Register / Record Date',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Book Closure page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Book Closure page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Book Closure page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Book Closure of Members Register', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/book-closure`, {
        method: 'POST',
        body: formData,
      });
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`book-closure-pdf-upload-${index}`}
            />
            <label
              htmlFor={`book-closure-pdf-upload-${index}`}
      ...
      {pageContent.showCmsPublishDate && (
        <div>
          ...
          <div className="relative group">
            <input
              type="text"
              value={doc.publishedDate || doc.date || ''}
              onChange={(e) => {
                const updatedDoc = { ...doc, publishedDate: e.target.value, date: e.target.value };
                if (index >= 0) {
                  const updatedSections = [...pageContent.sections];
                  updatedSections[0].documents[index] = updatedDoc;
                  setPageContent({ ...pageContent, sections: updatedSections });
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Book Closure page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Book Closure of Members Register / Record Date Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Book Closure page
            </p>
          </div>
                            ...
                            <div className="flex items-center gap-3 text-gray-400 font-bold text-[10px] uppercase tracking-widest">
                              {pageContent.showPublishDate && (
                                <div className="flex items-center gap-1.5 px-2 py-0.5 bg-gray-50 rounded group-hover:bg-blue-50 transition-colors">
                                  ...
                                  <span>{doc.publishedDate || doc.date || 'No Date'}</span>
                                </div>
                              )}
