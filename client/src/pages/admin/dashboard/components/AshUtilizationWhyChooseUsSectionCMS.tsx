...
export default function AshUtilizationWhyChooseUsSectionCMS() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await ashUtilizationCmsApi.deleteFeature(id);
  ...
  const handleSave = async () => {
    ...
    if (!editingFeature.description.trim()) {
    ...
    try {
      ...
      if (editingFeature.id) {
        await ashUtilizationCmsApi.updateFeature(editingFeature.id, editingFeature);
        setSuccess('Feature updated successfully');
      } else {
        await ashUtilizationCmsApi.createFeature(editingFeature);
  ...
  const handleToggleActive = async (feature: Feature) => {
    try {
      await ashUtilizationCmsApi.updateFeature(feature.id!, {
        ...feature,
  ...
  const handleOrderChange = async (feature: Feature, newOrder: number) => {
    try {
      await ashUtilizationCmsApi.updateFeature(feature.id!, {
        ...feature,
        order: newOrder,
                ...
                .map((feature) => (
                  <tr key={feature.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900 max-w-md truncate">{feature.description}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <input
                        type="number"
