...
interface CSRActivitiesPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorCSRActivitiesCMS() {
  const [pageContent, setPageContent] = useState<CSRActivitiesPageContent>({
    slug: 'csr-activities',
    title: 'CSR Activities',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'CSR Activities', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'csr-activities');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await investorsCmsApi.getPageContentBySlug('csr-activities');
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'CSR Activities', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load CSR Activities page');
      }
      setPageContent({
        slug: 'csr-activities',
        title: 'CSR Activities',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('CSR Activities page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('CSR Activities page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save CSR Activities page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'CSR Activities', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/csr-activities`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. CSR Activities 2023-24"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`csr-activities-pdf-upload-${index}`}
            />
            <label
              htmlFor={`csr-activities-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading CSR Activities page...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">CSR Activities Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the CSR Activities section
            </p>
          </div>
