...
interface CharterDocumentsPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorCharterDocumentsCMS() {
  const [pageContent, setPageContent] = useState<CharterDocumentsPageContent>({
    slug: 'charter-documents',
    title: 'Charter Documents',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'Charter Documents', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'charter-documents');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Charter Documents', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Charter Documents page');
      }
      setPageContent({
        slug: 'charter-documents',
        title: 'Charter Documents',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Charter Documents page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Charter Documents page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Charter Documents page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Charter Documents', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/charter-documents`, {
        method: 'POST',
        body: formData,
      });
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`charter-documents-pdf-upload-${index}`}
            />
            <label
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Charter Documents page...</p>
        </div>
      </div>
    );
  }
