...
interface StatementOfDeviationPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorStatementOfDeviationCMS() {
  const [pageContent, setPageContent] = useState<StatementOfDeviationPageContent>({
    slug: 'statement-of-deviation',
    title: 'Statement of Deviation',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'Statement of Deviation', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'statement-of-deviation');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await investorsCmsApi.getPageContentBySlug('statement-of-deviation');
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Statement of Deviation', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Statement of Deviation page');
      }
      setPageContent({
        slug: 'statement-of-deviation',
        title: 'Statement of Deviation',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Statement of Deviation page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Statement of Deviation page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Statement of Deviation page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Statement of Deviation', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/statement-of-deviation`, {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. Statement of Deviation 30/03/2024"
          />
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`statement-deviation-pdf-upload-${index}`}
            />
            <label
              htmlFor={`statement-deviation-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Statement of Deviation page...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-200">
                <i className="ri-file-warning-line text-white text-xl"></i>
              </div>
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Statement of Deviation Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Statement of Deviation section
            </p>
          </div>
