...
export default function InvestorDisclosureCMS() {
  ...
  const handlePdfUrlChange = async (url: string) => {
    ...
    if (isExternalPdfUrl(url)) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/download-pdf-from-url/disclosure`, {
          method: 'POST',
          headers: {
        ...
        {pageContent.showCmsPublishDate && (
          <div className="group">
            ...
            <input
              ...
              onChange={(e) => setEditingDocument(prev => prev ? {
                ...prev,
                document: { ...prev.document!, date: e.target.value }
              } : null)}
          ...
          {isManualPdfUrl ? (
            <div className="space-y-3">
              ...
              {!downloadingPdf && doc.pdfUrl && (
                <div className="flex items-center gap-2 px-3 py-2 bg-[#7cd244]/10 rounded-xl border border-blue-100/50 w-fit animate-in fade-in zoom-in duration-300">
                  <i className="ri-checkbox-circle-fill text-blue-500"></i>
                  <span className="text-[10px] font-bold text-[#3f7220] truncate max-w-[400px]">Active PDF: {doc.pdfUrl}</span>
                </div>
              )}
            </div>
          ) : (
              ...
              <div className={`absolute inset-0 border-2 border-dashed rounded-2xl flex items-center justify-center gap-4 transition-all ${uploadingPdf ? 'bg-[#7cd244]/10/50 border-blue-400' : 'bg-white border-gray-200 group-hover:border-blue-400 group-hover:bg-[#7cd244]/10/30'
                ...
                {uploadingPdf ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-black text-[#4f8f2a] uppercase tracking-widest">Uploading PDF...</span>
                  </div>
                ) : (
                  <>
    ...
    <div className="bg-white rounded-lg shadow-sm p-6" style={{ fontFamily: '"Open Sans", sans-serif' }}>
      ...
      <div className="mb-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Disclosure of Material Events CMS</h2>
            <p className="text-sm text-gray-400 mt-1 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#7cd244]/100"></span>
              Manage documents for Disclosure of Material Events or Information
            </p>
          </div>
          <button
            onClick={handleSave}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border-2 border-gray-100 p-8 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-12 h-12 bg-[#7cd244]/10 rounded-2xl flex items-center justify-center">
              <i className="ri-settings-4-line text-2xl text-[#4f8f2a] font-normal"></i>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-8">
            <div className="space-y-3">
              ...
              <input
                type="text"
                className="w-full bg-gray-50/50 border-2 border-gray-100 rounded-2xl py-4 px-6 font-bold text-gray-900 focus:bg-white focus:border-[#7cd244] focus:ring-0 transition-all placeholder:text-gray-300"
                value={pageContent.title}
                onChange={(e) => setPageContent({ ...pageContent, title: e.target.value })}
                placeholder="Disclosure of Material Events"
              />
            </div>
        ...
        <div className="bg-white rounded-3xl border-2 border-gray-100 p-8 shadow-sm relative overflow-hidden flex flex-col group">
          <div className="absolute top-0 right-0 p-8">
            <button
              ...
              className="w-10 h-10 bg-[#7cd244] text-white rounded-xl flex items-center justify-center hover:bg-[#6db038] transition-all shadow-lg shadow-blue-100 hover:scale-110 active:scale-95"
            >
          ...
          <div className="flex items-center gap-4 mb-2">
            <div className="w-10 h-10 bg-[#7cd244]/10 rounded-xl flex items-center justify-center group-hover:bg-[#7cd244]/20 transition-colors">
              <i className="ri-filter-line text-xl text-[#4f8f2a]"></i>
            </div>
              ...
              <div className="space-y-2">
                {pageContent.filterItems.map((item, index) => (
                  <div key={index} className="group/item flex items-center justify-between p-4 bg-gray-50/50 hover:bg-white border-2 border-transparent hover:border-blue-100 rounded-2xl transition-all cursor-default">
                    <span className="text-sm font-black text-gray-700 group-hover/item:text-[#4f8f2a] tracking-tight">FY {item}</span>
                    <div className="flex gap-2 opacity-0 group-hover/item:opacity-100 transition-all translate-x-4 group-hover/item:translate-x-0">
                      <button onClick={() => handleEditFilterItem(index)} className="w-8 h-8 bg-[#7cd244]/10 text-[#4f8f2a] rounded-lg hover:bg-[#7cd244] hover:text-white transition-all">
      ...
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-[#7cd244]/10 rounded-2xl flex items-center justify-center shadow-inner">
            <i className="ri-file-list-3-line text-2xl text-[#4f8f2a]"></i>
          </div>
          <div>
            <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">Disclosures</h3>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1 ml-1">Archive Manage</p>
          </div>
        </div>
        <button
          onClick={handleAddDocument}
          className="group flex items-center gap-3 px-6 py-4 bg-[#7cd244] text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] hover:bg-[#6db038] transition-all shadow-lg shadow-blue-100 active:scale-95 translate-y-2"
        >
      ...
      <div className="space-y-4 relative">
        {pageContent.documents.length === 0 ? (
          <div className="text-center py-24 bg-gray-50/50 rounded-[40px] border-4 border-dashed border-gray-100 flex flex-col items-center">
            ...
            <h4 className="text-xl font-black text-gray-400 uppercase tracking-widest">No disclosures found</h4>
              ...
              <div key={docIndex}>
                ...
                {editingDocument?.documentIndex === docIndex && (
                  <div className="mb-4 bg-white rounded-[32px] border-4 border-blue-600/10 p-1 shadow-2xl animate-in zoom-in-95 duration-300">
                    <div className="bg-[#7cd244] rounded-t-[28px] px-8 py-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                    ...
                    <div className="p-10 bg-white">
                      ...
                      <div className="mt-10 flex flex-col sm:flex-row gap-4 pt-8 border-t-2 border-gray-50">
                        <button
                          ...
                          className="flex-1 h-16 bg-[#7cd244] text-white rounded-2xl font-black text-sm uppercase tracking-[0.2em] hover:bg-[#6db038] transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-100 group active:scale-105"
                        >
                ...
                {/* Regular Card View */}
                {editingDocument?.documentIndex !== docIndex && (
                  <div className="group relative bg-white rounded-[2rem] border-2 border-gray-100 p-6 hover:border-[#7cd244] hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                      <div className="flex items-start gap-5">
                        ...
                        <div>
                          <h4 className="text-lg font-extrabold text-gray-900 group-hover:text-[#4f8f2a] transition-colors line-clamp-1">{doc.title}</h4>
                          <div className="flex flex-wrap gap-2 mt-2">
                            {doc.year && (
                              <span className="px-3 py-1 bg-[#7cd244]/10 text-[#4f8f2a] rounded-lg text-[10px] font-black uppercase tracking-widest border border-blue-100">
                                FY {doc.year}
                              </span>
                            )}
                            {doc.date && (
                      ...
                      <div className="flex items-center gap-3 self-end sm:self-center">
                        <button
                          ...
                          className="w-12 h-12 bg-[#7cd244]/10 text-[#4f8f2a] rounded-xl hover:bg-[#7cd244] hover:text-white transition-all duration-300 flex items-center justify-center shadow-sm"
                        >
      ...
      {showDocumentForm && editingDocument && editingDocument.documentIndex === -1 && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-xl z-[100] flex items-center justify-center p-4 sm:p-6 transition-all animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-4xl overflow-hidden border-8 border-white animate-in zoom-in-95 slide-in-from-bottom-12 duration-500">
            {/* Solid Blue Header */}
            <div className="bg-[#7cd244] px-10 py-8 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-[10px] font-bold uppercase tracking-widest mt-0.5">Add document to category: Disclosure</p>
                </div>
              </div>
              <button
                onClick={() => {
      ...
      {showFilterItemForm && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-xl z-[100] flex items-center justify-center p-4 transition-all animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-md overflow-hidden border-8 border-white animate-in zoom-in-95 duration-500">
            <div className="bg-[#7cd244] p-8 flex items-center justify-between">
            ...
            <div className="p-8">
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <input
                    type="text"
                    className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-4 px-6 font-bold text-gray-900 focus:bg-white focus:border-[#7cd244] focus:ring-0 transition-all placeholder:text-gray-300"
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-4 bg-[#7cd244] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#6db038] transition-all active:scale-95"
                  >
                    SAVE YEAR
                  </button>
                  <button
