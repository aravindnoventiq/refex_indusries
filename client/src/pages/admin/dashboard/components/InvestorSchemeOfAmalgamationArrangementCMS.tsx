...
interface SchemeOfAmalgamationArrangementPageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorSchemeOfAmalgamationArrangementCMS() {
  const [pageContent, setPageContent] = useState<SchemeOfAmalgamationArrangementPageContent>({
    slug: 'scheme-of-amalgamation-arrangement',
    title: 'Scheme of Amalgamation / Arrangement',
    hasYearFilter: false,
    filterItems: [],
    sections: [{ title: 'Scheme of Amalgamation Arrangement', documents: [] }],
    isActive: true,
    ...
    if (editingDocument) {
      ...
      if (isExternalPdfUrl(url)) {
        try {
          ...
          await investorsCmsApi.downloadAndSavePdf(url, 'scheme-of-amalgamation-arrangement');
        } catch (err) {
  ...
  const loadPageContent = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await investorsCmsApi.getPageContentBySlug('scheme-of-amalgamation-arrangement');
      if (data) {
        setPageContent({
          ...data,
          sections: data.sections || [{ title: 'Scheme of Amalgamation Arrangement', documents: [] }],
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Scheme of Amalgamation Arrangement page');
      }
      setPageContent({
        slug: 'scheme-of-amalgamation-arrangement',
        title: 'Scheme of Amalgamation / Arrangement',
  ...
  const handleSave = async () => {
    try {
      ...
      if (pageContent.id) {
        ...
        setSuccess('Scheme of Amalgamation Arrangement page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('Scheme of Amalgamation Arrangement page created successfully');
      }
      
      await loadPageContent();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to save Scheme of Amalgamation Arrangement page');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDocument = (doc: Document, index: number) => {
    ...
    if (!updatedSections[0]) {
      updatedSections[0] = { title: 'Scheme of Amalgamation Arrangement', documents: [] };
    }
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload/pdf/scheme-of-amalgamation-arrangement`, {
        method: 'POST',
        body: formData,
      });
  ...
  const renderDocumentFields = (doc: Document, index: number) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white rounded-xl border border-gray-100 shadow-sm">
      <div className="md:col-span-2">
        ...
        <div className="relative group">
          <input
            ...
            placeholder="e.g. RIL RGML – Composite Scheme of Amalgamation – September 2025"
          />
      ...
      <div>
        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Year (Optional)</label>
        <div className="relative group">
          <input
            type="text"
            value={doc.year || ''}
        ...
        {isManualPdfUrl ? (
          ...
          <div className="relative group">
            <input
              ...
              className="hidden"
              id={`scheme-pdf-upload-${index}`}
            />
            <label
              htmlFor={`scheme-pdf-upload-${index}`}
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] font-['Open_Sans'] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Scheme of Amalgamation Arrangement page...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-200">
                <i className="ri-file-merge-line text-white text-xl"></i>
              </div>
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Scheme of Amalgamation Arrangement Page CMS</h2>
            </div>
            <p className="text-sm text-gray-400 flex items-center gap-2 ml-1">
              ...
              Manage documents for the Scheme of Amalgamation Arrangement section
            </p>
          </div>
